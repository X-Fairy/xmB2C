<template>
	<view>
				...
				<view class="row" v-for="(row,index) in list" :key="index">
					<view>
						...
						<view class="type">订单号：{{row.order_sn}}({{row.tips}})
							<view class="icon shanchu" @tap="del(row)" v-if='row.order_state=="0"'></view>
						</view>
					...
					<view class="btns">
						<!-- <block v-if='row.order_state=="0"'>
							<view class="default" @tap="close(row)">取消订单</view>
						</block> -->
		...
		<modal :show="txtmodal" @cancel="txtmodal=false" :custom="true" :fadein="true" >
			<view class="modalCustom">
				<view class="prompt-title">退款原因</view>
				<input type="text" v-model="txt" class="content" placeholder="请输入退款原因" v-if="txtmodal"/>				
