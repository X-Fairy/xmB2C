<template>
	<view>
			...
			<view class="row" v-for="(row,index) in buylist" :key="index">
				<view class="goods-info" @tap="togoods(row)">
					<view class="img">
						...
						<view class="sign" v-if="row.isBOGO==1">
							买一送一
						</view>
					</view>
					<view class="info">
						<view class="title">{{row.pro_name}}</view>
						<view class="spec">数量:<text v-if="row.isBOGO==1">{{row.pronum*2}}</text>
							<text v-else>{{row.pronum}}</text>
							<text style="margin-left: 10rpx;">单价:{{row.pro_price}}</text>
						</view>
						<view class="price-number">
							<view class="price" v-if="row.dis_price">￥{{row.dis_price*(row.pronum-row.mm)}}
								<text class="dis_price">{{row.pro_price*row.pronum}}</text>
							</view>
							<view class="price" v-else-if="row.sec_price">￥{{row.sec_price*(row.pronum-row.mm)}}
								<text class="dis_price">{{row.pro_price*row.pronum}}</text>
							</view>
							<view class="price" v-else-if="row.bar_price">￥{{row.bar_price*(row.pronum-row.mm)}}
								<text class="dis_price">{{row.pro_price*row.pronum}}</text>
							</view>
							<view class="price" v-else-if="row.isBOGO==1">￥{{row.pro_price*(row.pronum-row.mm)}}
								<text class="dis_price">{{row.pro_price*row.pronum*2}}</text>
							</view>
							<view class="price" v-else-if="row.mm>0 && !row.dis_price">￥{{row.pro_price*(row.pronum-row.mm)}}
								<text class="dis_price">{{row.pro_price*row.pronum}}</text>
							</view>
		...
		<view class="detail">
			...
			<view class="" v-if="discountList.length>0">
				<view class="row" v-for="(item,index) in discountList" :key="index">
					<view class="nominal">
						{{item.disType}}
					</view>
					<view class="content">
		...
		<view class="footer">
			<view class="settlement">
				<view class="sum">
					<view class="money">合计:￥{{sumPrice}}</view>
					<view class="payLing">{{goodsPrice}}</view>
				</view>
				<view class="btn" @tap="toPay">支付</view>
			</view>
		</view>
		<modal :show="paySucess" :custom="true" maskClosable="false"> 
			<view class="pop">
	...
	export default {
		...
		methods: {
			...
			loadData() {
				...
				this.$xm.post('/cart/order', data, res => {
					if (res.s == 0) {
						...
						this.tips = res.tips;
						this.goodsPrice = Number(res.allprice);
						let arr = [];
						for (let i in res.discount_list) {
							let money = res.discount_list[i];
							switch (i) {
								case 'BOGO':
									i = '买一送一';
									break;
			...
			discounts(goodsList) {
				...
				this.$xm.post('/Coupon/match_optimalss', data, res => {					
					...
					if (res.data) {
						...
						this.mjPrice = res.data.cut;
						this.mjtype = res.data.type;
						this.getship()
					}
			...
			shipChange(e) {
				...
				// 优惠价
				this.sumPrice = (this.payLing + this.freight).toFixed(2);
