	...
	<view>
			...
			<view class="list">
				...
				<view class="box" v-for="(row,index) in orderList" :key="index" @tap="toOrderList(row.id)">
					<view class="img">
						...
						<text class="num" v-if="row.num>0&&row.id!=8">{{row.num}}</text>
					</view>
		...
		<tui-modal :show="showModal" :custom="true" :fadein="true">
			<view class="tui-modal-custom">
				...
				<!-- 字节跳动 -->
				<button type="warn" open-type="getUserInfo" @getuserinfo="getuserinfo" withCredentials="true" style="margin: 20rpx 0;"
				 :class="userProvider=='weixin' ? '':'accredit'">授权登录</button>
			</view>
