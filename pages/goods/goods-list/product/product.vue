				...
				<view class="product" v-for="(goods,index) in goodsList" :key="index" v-if="goodsList.length>0">
					...
					<view class="info">
						<view class="price" v-if="goods.dis_price" @tap="toGoods(goods)">
							...
							<text class="dis_price" v-if="goods.dis_price!==undefined" style="color: #B2B2B2;font-size: 24rpx;padding-left: 10rpx;text-decoration: line-through;">{{ goods.pro_price }}</text>
						</view>
						<view class="price" v-else @tap=" toGoods(goods)">
							<text style="font-size: 24rpx;">¥</text>{{ goods.pro_price }}
						</view>
						<image src="../../../../static/img/cart.png" mode="" class="cart" @tap="joinCart(goods)" v-if="goods.num!='0'"></image>
