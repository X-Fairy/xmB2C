...
">
		...
		<view class="info-box one-info">
			<view class="goods-info">
				...
				<view class="price" v-if="goodsData.group_price"><text style="font-size: 26rpx;">拼团价</text>￥{{goodsData.group_price}}<text
					 style="font-size: 28rpx;text-decoration: line-through;color: #b2b2b2;margin-left: 15rpx;">{{goodsData.price}}</text></view>
		...
		<view class="info-box spec">
			...
			<view class="" v-if="couponList.length>0" style="border-bottom: 1px solid #eee;">
				<view class="row">
					<view class="text" v-if="couponList.length>0">全场通用（组合套装和秒杀商品除外）</view>
				</view>				
				<view class="" v-if="couponList.length>0" style="display: flex;justify-content: space-between;flex-wrap: wrap;align-items: center;padding: 25rpx 0;">
				...
				<view class="row" style="border-bottom: 0;display: block;text-align: center;" v-if="dismsg">
					<view class="text mm" v-for="(ele,index) in dismsg" :key="index">{{ele}}</view>
				</view>
			</view>
			<view class="row">
		...
		methods: {
			...
			getDetails() {
				...
				this.$xm.post('/Product/getInfo', data, (res) => {
					...
					if(result.dismsg){
						this.dismsg=result.dismsg[0].split(',');
						this.dismsg=action;
					}
					if(result.discount){
			...
			joinGroup() {
				...
				if (this.group.type == 'join') {
					...
					// 开团					
					uni.navigateTo({
						url: '../order/confirmation?store=' + store + '&proid=' + this.gid + '&pronum=' + this.value + '&type=group' +
							'&sid=' + this.group_gid
					})
				}
