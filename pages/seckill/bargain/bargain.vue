<template>
	<view>
		<view class="group" style="padding-bottom: 50upx;" v-if="tabIndex==0">
			...
			<view class="goods">
				<view v-for="(item, index) in goodsList" :key="index" class="goods-item" @tap="toagainCut(item)">
					<view class="left">
						<image :src="item.pro_img" mode=""></image>
					</view>
					<view class="right">
						<view class="info" style="height: 52%;">{{item.pro_name}}</view>
						<view class="tip">
							<view class="content">
								<!-- <text>{{item.bargain_num}}人砍</text> -->
								<!-- <text>包邮</text> -->
							</view>
							<view class="" style="display: flex;justify-content: left;align-items: center;padding-right: 10rpx;" v-if="item.bargain_num<=5">
		...
		<view class="goods" v-else>
			<view v-for="(item, index) in bartainlist" :key="index" class="goods-item" v-if="bartainlist.length>0">
				...
				<view class="right">
					...
					<view class="progress" v-if="item.etime>0 && item.progress<100">
					...
					<view class="bottom">
						<view class="" v-if="item.etime<=0">
							<view class="success" v-if="item.progress>=100">
			...
			<view class="loading-text" v-else>暂无参与砍价的商品</view>
		</view>
		<view class="tabBar">
		...
		methods: {
				...
				this.$xm.post('/bargain/get_bartain_list', "", res => {
					if (res) {
						res.forEach((ele) => {
							...
							ele.progress = ele.price_totle / (ele.pro_price-ele.dis_price) * 100
			...
			toagainCut(e){				
				if (e.xopenid && !e.order_sn) {
					...
					this.$xm.post('/bargain/get_bartain_num','',(res)=>{
						console.log(res);
						if(res>1){
							uni.showToast({
								title: '你今日发起砍价商品已达上限，明天再来吧！',
								icon: 'none'
							})
						}else{
							uni.navigateTo({
