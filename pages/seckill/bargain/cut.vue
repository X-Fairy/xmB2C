<template>
	<view>
		<view class="pageInfo">
			<view class="rule" @tap="ruleShow=true">
				活动规则
			</view>
			<view class="product">
		...
		<modal :show="ruleShow" @cancel="ruleShow=false" :custom="true" class="rulecontent">
			<scroll-view  scroll-y="true" class="ruleInfo" >
				一.活动规则</br>
				1.用户发起砍价后可自己砍一刀，邀请好友可帮忙砍价；</br>
				2.同一用户每天最多可发起2个不同商品的砍价（同一用户名、手机号、微信id、移动设备、银行卡、支付账户等均认为同一用户），单个商品活动时间内仅能允许发起一次;</br>
				3.单个用户每天最多帮忙砍价2次，且只能给同一熙美诚品账户的同一个砍价商品帮忙砍一次；每次砍价金额随机；</br>
				4.砍价商品库存有限，以砍价成功的顺序先到先得。</br>		
				三，其他注意事项</br>
				1.在参与活动期间，遇到任何问题，可及时联系客服反馈解决​；</br>
				2.本次活动的最终解释权为熙美诚品所有。</br>
			</scroll-view>
		...
		methods: {
			...
			getinfo() {
				...
				this.$xm.post('/bargain/getinfo', data, (res) => {
					...
					this.goods.progress = res.price_totle / (this.goods.pro_price-this.goods.dis_price) * 100
